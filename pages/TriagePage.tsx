...
import { Send, Bot, User as UserIcon, AlertCircle, ChevronRight, Loader2, Image as ImageIcon, X } from 'lucide-react';
      ...
      {result && (
        <div className="absolute inset-x-4 bottom-24 z-10 animate-in slide-in-from-bottom-5 fade-in duration-500">
           <div className={`bg-white dark:bg-gray-900 rounded-2xl shadow-xl border overflow-hidden ${
             result.level === 'Red' ? 'border-red-500' : 
             result.level === 'Yellow' ? 'border-amber-400' : 'border-blue-500'
           }`}>
              <div className={`p-3 text-white font-bold flex items-center gap-2 ${
                result.level === 'Red' ? 'bg-red-600' : 
                result.level === 'Yellow' ? 'bg-amber-500' : 'bg-blue-600'
              }`}>
                 <AlertCircle size={20} />
